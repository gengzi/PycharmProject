...
def postStudent(session,beforTimestamp,afterTimestamp,topicId):
    logger.info("开始的时间戳:"+beforTimestamp)
    logger.info("结束的时间戳:"+afterTimestamp)
    ...
    logger.info("该分页数据的时间：" + formatTime)
    logger.info("栏目编号：" + topicId)
    logger.info("时间戳：" + timestamp)
    logger.info("整个数据的日期：" +formatTimeday)
    # 目录
...
if __name__ == "__main__":
    ...
    # 调用请求数据方法
    logger.info(str(yesterday)+"抽取数据开始>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

    # [2.1] 将五个模块的数据都抓取出来
    topicIds = gettopicIds()
    for i in topicIds:
        try:
            logger.info("爬取的栏目编号:"+str(i))
            postStudent(session,beforTimestamp,afterTimestamp,str(i))
        except Exception,e:
            logger.error(e)
            logger.error("栏目编号:"+str(i)+"爬取出错")
            continue
    logger.info("<<<<<<<<<<<<<<<<<<<<<<爬取帖子，抽取数据结束>>>>>>>>>>>>>>>>>>>>>>>>")
